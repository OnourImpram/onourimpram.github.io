...
def verify_package(root:Path)->dict:
    dist=root/'dist';manifest=json.loads((dist/'release-v10.json').read_text(encoding='utf-8'))
    if manifest.get('release')!='v10-refined-atelier' or manifest.get('indexable') is not False:
...
def publish(repo:Path,package:Path)->str:
    ...
    branch='elif-v10-release-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    git(repo,'switch','-c',branch,'origin/main')
    temporary=Path(tempfile.mkdtemp(prefix='.elif-v10-staging-',dir=repo))
    try:
        shutil.copytree(package/'dist',temporary,dirs_exist_ok=True)
        source=temporary/'source-v10';source.mkdir()
        ...
        git(repo,'commit','-m','release(elif): V10 dual-sided Three.js atelier and editorial refinement')
        ...
        print('Check the public release-v10.json and the Three.js studio before announcing the release.')
