...
export function evaluateRoomFit(roomWidth:number,roomDepth:number,footprint:{width:number;depth:number}):RoomFitResult|null{
 ...
 return {fits:widthClearance>=0&&depthClearance>=0,widthClearance,depthClearance,footprintWidth:n(footprint.width),footprintDepth:n(footprint.depth),roomWidth:n(roomWidth),roomDepth:n(roomDepth)};
}
