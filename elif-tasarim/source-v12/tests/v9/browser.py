"""Offline V9 browser tests. Same system Chromium, native WebGL through Xvfb.
...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'evidence/v9/browser';OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 ...
 def snap(name,width=1920,height=1280):
  ...
  return raw
 check('V9 identity, approved headline and eight category entrances',page.locator('.preview-bar').inner_text().startswith('V9') and 'Zamana değer' in page.locator('h1').inner_text() and page.locator('.v9-category-ribbon>a').count()==8)
 ...
 page.get_by_role('button',name='Bu tasarımı Yunus Usta ile konuş',exact=True).click();page.get_by_role('button',name='Devam et',exact=True).click()
