...
test('V11 has one active reproducible build and pinned engine/compiler',()=>{
 const p=JSON.parse(fs.readFileSync('package.json'));assert.equal(p.version,'0.13.0');assert.equal(p.scripts.build,'node tools/build-v13.cjs');assert.equal(p.devDependencies.typescript,'5.8.3');
...
test('clean V11 builds reproduce every publication byte and include local room/engine',()=>{
 cp.execFileSync(process.execPath,['tools/build-v13.cjs']);const a=fs.readFileSync('dist/release-v13.json','utf8'),m=JSON.parse(a);
 assert.equal(m.release,'v13-final-atelier');assert.equal(m.routes.length,43);assert.equal(m.indexable,false);
 ...
 cp.execFileSync(process.execPath,['tools/build-v13.cjs']);assert.equal(fs.readFileSync('dist/release-v13.json','utf8'),a);
...
test('offline preview includes lazy native ESM without third-party CDN or customer data',()=>{
 const html=fs.readFileSync('preview/Elif_Tasarim.html','utf8');assert.ok(html.includes('__ELIF_LOAD_3D__'));assert.ok(html.includes('sol-rafli-kitaplik'));assert.ok(html.includes('V13 FINAL / TASARIM'));assert.ok(!html.includes('V8 / TASARIM'));assert.ok(!/<script[^>]+src=["']https:\/\/(?:unpkg|cdn)/i.test(html));
});
...
test('no distributed font file exists in publication',()=>{
 const m=JSON.parse(fs.readFileSync('dist/release-v13.json'));assert.ok(!Object.keys(m.files).some(x=>/\.(woff2?|ttf|otf)$/i.test(x)));
});
