...
with sync_playwright() as p:
 ...
 def material():
  page.get_by_role('button',name='Açık meşe tonu',exact=True).click();page.get_by_role('button',name='Bu tasarımı Yunus Usta ile konuş',exact=True).click();page.get_by_role('button',name='Devam et',exact=True).click();value=page.locator('label').filter(has_text='Malzeme yaklaşımı').locator('select').input_value();record('Studio finish is a visible selected option in request form','meşe' in value.lower(),{'selectValue':value});values=page.locator('main select').evaluate_all('(xs)=>xs.map(x=>({value:x.value,index:x.selectedIndex}))');record('All request dropdowns have meaningful selected values',all(x['index']>=0 and x['value'] for x in values),values)
