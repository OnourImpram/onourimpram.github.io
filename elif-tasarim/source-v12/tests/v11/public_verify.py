...
m=json.loads((R/'dist/release-v20.json').read_text());expected=(R/'dist/release-v20.json').read_bytes()
def get(path):
 with urlopen(Request(B+path,headers={'User-Agent':'Elif-Release-Verification/11'}),timeout=45) as r:return r.read()
for n in range(36):
 try:
  if get('release-v20.json')==expected:break
...
for path in m['routes']:
 ...
 pages.append({'path':path,'v11':'v20-master-atelier'in data,'noindex':'noindex,nofollow'in data,'legacyName':bool(re.search('Yusuf',data)),'description':description})
