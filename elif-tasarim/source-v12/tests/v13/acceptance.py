...
with sync_playwright() as p:
 ...
 try:
  ...
  page.get_by_role('button',name='Detay noktalarını göster',exact=True).click();page.wait_for_timeout(250)
  visible=page.locator('.v13-hotspot:not([hidden])').count();assert visible>=2,visible;rec('02. Projected 3D detail hotspots are visible',{'visible':visible})
  page.locator('.v13-hotspot:not([hidden])').first.click();page.wait_for_selector('.v13-hotspot-card');assert page.locator('.v13-hotspot-card').is_visible();rec('03. Hotspot opens a decision-oriented detail card')
