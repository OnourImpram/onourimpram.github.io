...
export function createAtelierRoom(options={}) {
 ...
 let disposed=false,selection='both',power=65,cutaway=false;
 const architecture=new THREE.Group();architecture.name='oda-mimarisi';root.add(architecture);
 ...
 const wall=box(12,6,.045,[0,2.6,-1.52],architecture,stone,'tas-duvar',.003);
 for(const sign of [-1,1])box(.025,2.9,.025,[sign*.62,1.49,-1.485],architecture,trim,'ince-duvar-detayi',.002);
 // Central wall typography uses an optional local canvas texture, never a bundled font file.
 if(options.brand){const m=new THREE.MeshBasicMaterial({map:options.brand,transparent:true,depthWrite:false});owned.add(m);mesh(new THREE.PlaneGeometry(.67,.51),m,architecture,[0,1.81,-1.483],'elif-duvar-imzasi')}
 ...
 box(1.85,.034,.060,[0,2.60,-.24],architecture,brass,'pirinc-sarkit');
 box(1.78,.005,.041,[0,2.581,-.24],architecture,lamps,'sarkit-isigi',.001);
 for(const sign of [-1,1])cylinder(.002,.002,.45,[sign*.68,2.84,-.24],architecture,trim);
 // Upholstered chair behind the desk. Staging, not an implied sales inclusion.
 const chair=new THREE.Group();chair.name='calisma-koltugu';chair.position.set(-.10,0,-.81);root.add(chair);
 ...
 function setShelves(v){selection=['both','left','right','none'].includes(v)?v:'both';left.visible=!cutaway&&(selection==='both'||selection==='left');right.visible=!cutaway&&(selection==='both'||selection==='right');const lp=root.getObjectByName('sol-bitki'),rp=root.getObjectByName('sag-bitki');lp.visible=left.visible;rp.visible=right.visible;localLights[0].visible=left.visible;localLights[1].visible=right.visible;}
 function setCutaway(v){cutaway=!!v;architecture.visible=!cutaway;chair.visible=!cutaway;setShelves(selection);}
 ...
 function stats(){let meshes=0;root.traverse(o=>{if(o.isMesh)meshes++});return {cutaway,architectureVisible:architecture.visible,chairVisible:chair.visible,chairPosition:chair.position.toArray(),shelves:8,visibleBookcases:Number(left.visible)+Number(right.visible),selection,shelfLight:power,activeShelfLights:localLights.filter(l=>l.visible&&l.intensity>0&&root.visible).length,meshes,disposed,visible:root.visible}}
 ...
 if(options.batch!==false){batchStatic(left);batchStatic(right);batchStatic(architecture);batchStatic(chair);batchStatic(root,new Set([architecture,chair,left,right,root.getObjectByName('sol-bitki'),root.getObjectByName('sag-bitki')]));}
 setShelves('both');setLight(65);
 return {root,left,right,wall,chair,setCutaway,setShelves,setLight,setVisible:v=>root.visible=!!v,stats,dispose};
}
