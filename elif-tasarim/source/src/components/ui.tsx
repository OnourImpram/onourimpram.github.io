...
export function Photo({ name, alt, ratio = '', className = '', caption = true, eager = false }: {
    ...
    caption?: boolean;
    eager?: boolean;
}) { return <figure className={'photo ' + className} style={ratio ? { aspectRatio: ratio } : undefined}><img src={image(name)} alt={alt} loading={eager ? 'eager' : 'lazy'} decoding="async"/>{caption && <figcaption>Temsili tasarım görseli</figcaption>}</figure>; }
