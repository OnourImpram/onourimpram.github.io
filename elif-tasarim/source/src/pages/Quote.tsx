...
import { validateQuoteStep, validateFile, parseDimension, safeDraft, readLocal, writeLocal, downloadText } from '../lib/domain';
...
export class Quote extends Component<PageProps & {
    ...
    field = (name: keyof Values, label: string, placeholder = '', type = 'text') => <label className="form-field" htmlFor={'q-' + name}><span id={'label-' + name}>{label}</span><input aria-labelledby={'label-' + name} id={'q-' + name} type={type} value={String(this.state.v[name])} maxLength={name === 'name' ? 100 : 200} placeholder={placeholder} onInput={e => this.set(name, e.currentTarget.value)} aria-invalid={!!(this.state.errors as any)[name]} aria-describedby={(this.state.errors as any)[name] ? 'err-' + name : undefined}/>{(this.state.errors as any)[name] && <small className="field-error" id={'err-' + name}>{(this.state.errors as any)[name]}</small>}</label>;
    renderStep() {
        const { v, step, files } = this.state;
        if (step === 0)
            return <div className="choice-grid" id="q-kind" tabIndex={-1}>{[['Yemek masası', 'dining.webp'], ['Sandalye', 'chair.webp'], ['Konsol', 'sideboard.webp'], ['Yükseklik ayarlı masa', 'office.webp']].map(([kind, photo]) => <button type="button" key={kind} className={'picture-choice ' + (v.kind === kind ? 'selected' : '')} aria-pressed={v.kind === kind} onClick={() => this.set('kind', kind)}><img src={image(photo)} alt=""/><span>{kind}<i>{v.kind === kind ? <Icon name="check" size={14}/> : null}</i></span></button>)}<button type="button" className={'choice-wide ' + (v.kind === 'Birlikte karar verelim' ? 'selected' : '')} onClick={() => this.set('kind', 'Birlikte karar verelim')} aria-pressed={v.kind === 'Birlikte karar verelim'}><Icon name="ruler"/>Başka bir fikir / Birlikte karar verelim<Icon name="arrow"/></button>{(this.state.errors as any).kind && <p className="field-error">{(this.state.errors as any).kind}</p>}</div>;
        if (step === 1)
            return <><label className="check-card"><input type="checkbox" checked={v.unknown} onChange={e => this.set('unknown', e.currentTarget.checked)}/><span>Ölçülerimi birlikte belirleyelim<small>Şimdilik kesin ölçü vermeden devam edebilirsiniz.</small></span></label>{!v.unknown && <><div className="form-row three">{this.field('width', 'En', '180')}{this.field('depth', 'Derinlik', '90')}{this.field('height', 'Yükseklik', '75')}</div><label className="form-field">Ölçü birimi<select value={v.unit} onChange={e => this.set('unit', e.currentTarget.value)}><option value="cm">Santimetre (cm)</option><option value="mm">Milimetre (mm)</option></select></label><p className="small muted">120,5 veya 120.5 yazabilirsiniz. Bunlar ilk talep ölçüleridir; üretim onayı değildir.</p></>}<details className="inline-guide"><summary>Ölçü alma notları <Icon name="plus" size={16}/></summary><p>En, derinlik ve yüksekliği ayrı ölçün. Seçtiğiniz birimi bütün alanlarda tutarlı kullanın. Kapı, çekmece ve sandalye için gereken kullanım paylarını atölyeyle değerlendirin. İlk ölçüler üretim onayı değildir.</p></details></>;
        if (step === 2)
