...
const inRange=(raw:string|null,min:number,max:number,fallback:number)=>{if(raw===null||raw.trim()==='')return fallback;const n=Number(raw.replace(',','.'));return Number.isFinite(n)&&n>=min&&n<=max?Math.round(n):fallback}
