...
export function createDeskScene(host,initial={},hooks={}){
 ...
 const controls=new OrbitControls(camera,canvas);controls.target.set(.10,.64,.22);controls.enableDamping=true;controls.dampingFactor=.09;controls.enablePan=false;controls.enableZoom=false;controls.minDistance=2;controls.maxDistance=14;controls.minPolarAngle=.15;controls.maxPolarAngle=Math.PI/2-.07;controls.rotateSpeed=.6;controls.touches.ONE=THREE.TOUCH.ROTATE;controls.touches.TWO=THREE.TOUCH.DOLLY_PAN;
 // Wheel remains ordinary page scrolling, including on mobile.
 canvas.style.touchAction='pan-y';controls.addEventListener('change',invalidate);controls.addEventListener('start',()=>{automaticFrame=false;autoRotate=false;hooks.motion?.(false);hooks.camera?.('custom')});
 ...
 function syncRoom(){room.setVisible(target.room==='atelier'&&viewName!=='top');room.setShelves(target.shelves);room.setLight(target.shelfLight);ring.visible=target.room==='product';contact.position.y=target.room==='atelier'?.008:-.007;controls.minAzimuthAngle=target.room==='atelier'?-1.10:-Infinity;controls.maxAzimuthAngle=target.room==='atelier'?1.10:Infinity;}
 ...
 function frameModel(){
  ...
  const isRoom=target.room==='atelier'&&viewName!=='top';
 ...
 function render(){if(dead||lost||!visible||document.hidden)return;controls.update();projectLabels();renderer.render(scene,camera);frameCount++;}
 ...
 function view(name='perspective'){autoRotate=false;hooks.motion?.(false);hooks.camera?.(name);viewName=name;automaticFrame=name!=='detail';
  if(name==='top'&&target.room==='atelier'){room.setVisible(false)}else room.setVisible(target.room==='atelier');
  ...
  else if(name==='left')camera.position.set(-2.15,1.75,4.1*wide);
  else if(name==='right')camera.position.set(2.15,1.75,4.1*wide);
 ...
 function zoom(f){automaticFrame=false;const v=camera.position.clone().sub(controls.target);const len=THREE.MathUtils.clamp(v.length()*f,2,14);v.setLength(len);camera.position.copy(controls.target).add(v);invalidate()}
function applyLight(mode){theme=mode==='evening'?'evening':'day';target.lighting=theme;const dusk=theme==='evening';scene.background.set(dusk?'#e0d1bd':'#eee7dc');scene.fog.color.copy(scene.background);floorMat.color.set(dusk?'#d9cbb4':'#e6dfd3');sun.color.set(dusk?0xffd497:0xffe8c3);sun.intensity=dusk?1.6:2.7;ambient.intensity=dusk?.60:.82;renderer.toneMappingExposure=dusk?.92:.96;invalidate()}
 ...
 const api={
  ...
  rotate(on){automaticFrame=false;autoRotate=!!on;hooks.motion?.(autoRotate);invalidate()},
  light:applyLight,
  snapshot(width=1920,height=1280){
   ...
   try{settle();renderer.setPixelRatio(1);renderer.setSize(width,height,false);camera.aspect=width/height;camera.updateProjectionMatrix();frameModel();renderer.render(scene,camera);return canvas.toDataURL('image/png')}
   finally{renderer.setPixelRatio(ratio);renderer.setSize(size.x,size.y,false);camera.aspect=aspect;camera.position.copy(position);camera.quaternion.copy(rotation);controls.target.copy(aim);automaticFrame=wasAutomatic;camera.updateProjectionMatrix();projectLabels();invalidate()}
  },
