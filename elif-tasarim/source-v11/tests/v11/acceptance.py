...
with sync_playwright() as p:
 ...
 b=p.chromium.launch(executable_path=exe if Path(exe).exists() else None,headless=False,args=['--no-sandbox','--disable-dev-shm-usage','--use-angle=swiftshader','--enable-unsafe-swiftshader'])
 ...
 try:
  fresh();assert page.locator('.preview-bar').inner_text().startswith('V11');assert 'Zamana değer' in page.locator('h1').inner_text();assert 'Yusuf' not in page.locator('body').inner_text();assert page.locator('.v9-category-ribbon').count()==1;assert page.locator('.home-categories').count()==0;assert page.locator('canvas').count()==0
