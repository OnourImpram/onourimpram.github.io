...
import json
R=Path(__file__).resolve().parents[2];O=R/'evidence/v9';rows=[];errors=[]
with sync_playwright() as p:
 ...
 page=b.new_page();page.on('pageerror',lambda e:errors.append(str(e)));page.emulate_media(reduced_motion='reduce');page.set_content((R/'preview/Elif_Tasarim.html').read_text(),wait_until='load')
 routes=json.loads((R/'dist/release-v9.json').read_text())['routes']
 ...
 for route in ['/','/modelini-getir','/tasarim-masasi']:
  page.set_content((R/'preview/Elif_Tasarim.html').read_text(),wait_until='load');page.set_viewport_size({'width':390,'height':844});page.evaluate('(r)=>location.hash="#"+r',route);page.wait_for_timeout(200)
