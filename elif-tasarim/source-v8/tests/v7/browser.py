...
with sync_playwright() as pw:
 ...
 def files():
  ...
  with zipfile.ZipFile(OUT/'sample-project.zip') as z:check(z.testzip() is None,'Bad ZIP');check(len(z.namelist())==2,'Attachment not in bundle');check(z.read('Elif_Proje_Ozeti.txt').decode().startswith('Merhaba Yusuf Usta'),'Summary not included');return {'zipFiles':len(z.namelist()),'nativeShare':'Abort simulated, no real third-party send'}
