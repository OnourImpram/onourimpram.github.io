...
export function publicHref(path: string) { if(typeof window==='undefined')return path; const w=window as any; if(w.__ELIF_PREVIEW__)return '#'+path; const [p,qs]=path.split('?');return (w.__ELIF_BASE__||'')+(p==='/'?'/':p.replace(/\/+$/,'')+'/')+(qs?'?'+qs:''); }
