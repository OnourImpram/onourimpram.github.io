...
export function createDeskScene(host,initial={},hooks={}){
 ...
 const api={
  update(next){if(!anim)last=performance.now();const old=target;target=normalized(next);if(old.width!==target.width||old.depth!==target.depth)build();else if(old.material!==target.material)woodMeshes.forEach(m=>m.material=woods[target.material]);atelier.setMaterial(target.material);if(matchMedia('(prefers-reduced-motion: reduce)').matches){current={height:target.height,angle:target.angle,drawer:target.drawers?1:0,door:target.door?1:0};apply();}invalidate()},
  setView:view,zoom,
