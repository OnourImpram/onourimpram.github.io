"""Public-only runtime sample. No submissions, account access, asset reuse or private API probing."""
import json
...
NAV="[...document.querySelectorAll('header a,nav a')].map(a=>{try{return {text:(a.textContent||'').trim().slice(0,60),path:new URL(a.getAttribute('href')||'',document.baseURI).pathname}}catch{return null}}).filter(Boolean).slice(0,35)"
MEASURE="""()=>{const keys=['fontFamily','fontSize','lineHeight','letterSpacing','color','backgroundColor','display','gap','maxWidth','padding','transitionDuration'];return {title:document.title,viewport:innerWidth,height:innerHeight,elements:[...document.querySelectorAll('h1,h2,header,nav,main,button')].filter(e=>e.getBoundingClientRect().width>0).slice(0,24).map(e=>{const c=getComputedStyle(e),r=e.getBoundingClientRect();return {tag:e.tagName,rect:{x:r.x,y:r.y,width:r.width,height:r.height},style:Object.fromEntries(keys.map(k=>[k,c[k]]))}}),navigation:NAVIGATION}}""".replace('NAVIGATION',NAV)
records=[]
with sync_playwright() as p:
 browser=p.chromium.launch(headless=True)
 fixture=browser.new_page()
 fixture.set_content('<base href="https://example.com/"><header><a href="/html">HTML</a><svg><a href="/svg">SVG</a></svg><a href="../relative">Relative</a></header>')
 assert [v['path'] for v in fixture.evaluate('()=>'+NAV)]==['/html','/svg','/relative']
 fixture.close()
 print('PASS SVG and relative-link navigation regression')
 for name,url in SITES:
  ...
  try:
   ...
   rec['status']='OBSERVED'
  except ValueError as e:rec['status']='PARTIAL' if rec['measurements'] else 'BLOCKED';rec['reason']=str(e)[:250]
  except Exception as e:rec['status']='PARTIAL' if rec['measurements'] else 'ERROR';rec['reason']=str(e)[:250]
